...
| 03 白板图解 | **必有**：一张精确几何 SVG 说清算法全貌，严格按 `template/viz/whiteboard.html` 的三段式布局和防重叠规则画（初始状态 → 关键动作 → 结论，四色语义，禁用滤镜） |
| 04 可视化 | 至少一个：SVG 步骤机（见第 5 步），讲推演细节 |
| 05 数据结构原理 | 涉及哈希/堆/并查集/单调栈/平衡树等必须讲原理；纯技巧题可省 |
| 06 代码实现 | `tabs` 多解法（暴力 + 最优，有变种再加），最优解 tab 名带 ★ 且默认 active；手写高亮 span（`tok-k/f/s/c/n/b`） |
| 07 复杂度 | `cx` 表全解法对比，最优行 `tr.best` |
| 08 边界与变体 | 边界 case 列表 + 变体题（带 LC 题号 + 一句为什么不同） |
| 页脚 | `ALGO · <主题>专题` |
...
- [ ] 白板图解：无滤镜、形状都 `fill="none"`、四色语义正确、文字 ≥12px 且包围盒互不重叠、全屏可点
